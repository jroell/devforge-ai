...
import { Code, Eraser, ArrowRightFromLine } from 'lucide-react'
...
function looksEncoded(text: string): boolean {
  return /&(?:amp|lt|gt|quot|#\d+|#x[\da-f]+);/i.test(text)
}

export default function HtmlEntitiesView() {
  ...
  const process = useCallback((text: string, currentMode: 'encode' | 'decode') => {
    ...
    try {
      const result = currentMode === 'encode' ? encodeHtmlEntities(text) : decodeHtmlEntities(text)
      setOutput(result)
      setError(null)
    } catch (e) {
      setOutput('')
      setError(currentMode === 'decode' ? 'Invalid HTML entities' : 'Failed to encode input')
    }
  }, [])
        ...
        <TabsContent value="encode" className="flex min-h-0 flex-1 flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <Textarea
              ...
              placeholder='Enter text with characters like <, >, &, ", etc...'
          ...
          <OutputPanel label="Encoded Output" value={output} placeholder="Encoded output will appear here..." />
        </TabsContent>

        <TabsContent value="decode" className="flex min-h-0 flex-1 flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <span className="text-sm font-medium">HTML Entities Input</span>
            <Textarea
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Enter HTML entities like &amp;lt; &amp;gt; &amp;amp;..."
              className="min-h-[120px] font-mono text-sm"
            />
          </div>
          {error && (
